...
func TestAll(t *testing.T) {
	...
	for _, tf := range testFiles {
		t.Run(tf, func(t *testing.T) {
			...
			// we don't support any remote servers in shell tests (those are in metrocli unit tests)
			// METROCLI_TM_ADDR must be unset
			cmd.Env = append(os.Environ(), "METROCLI_TM_ADDR=")
...
func ensureBlogCLIBinary(t testing.TB) {
	...
	if cmd := exec.Command("metrocli", "version"); cmd.Run() != nil {
		t.Skipf(`metrocli binary is not available
