...
// RunMetrod is like RunTendermint, just executes the metroexecutable, assuming a prepared home directory
// This package will be removed with weave 0.22.0 since it is a copy-paste from the repo
func RunMetrod(ctx context.Context, t tmtest.TestReporter, home string) (cleanup func()) {
	t.Helper()

	appPath, err := exec.LookPath("metro")
	if err != nil {
		if os.Getenv("FORCE_TM_TEST") != "1" {
			t.Skip("metro binary not found. Set FORCE_TM_TEST=1 to fail this test.")
		} else {
			t.Fatalf("%s binary not found. Do not set FORCE_TM_TEST=1 to skip this test.", "metro")
		}
	}
	...
	if err := cmd.Start(); err != nil {
		t.Fatalf("%s process failed: %s", "metro", err)
	}
